...
export class BookId extends ValueObject<string, 'BookId'> {
    ...
    toISBN(): string {
        ...
        if (this._value.length ===10) {
            ...
            const publisherIdentifier = this._value.substring(1, 3); // 出版社コード（2桁）
            ...
            const publisherCode = this._value.substring(4, 6); // 出版社コード（2桁）
            const bookCode = this._value.substring(6, 12); // 書名コード（6桁）
