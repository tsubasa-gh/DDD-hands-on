...
describe('BookId', () => {
    ...
    test('有効なフォーマットの場合正しい変換結果を期待', () => {
        expect(new BookId('9784167158057').getValue()).toBe('9784167158057');
        expect(new BookId('4167158051').getValue()).toBe('4167158051');
    });
    ...
    test('不正な文字数の場合にエラーを投げる', () => {
        // 境界値のテスト
        expect(() => new BookId('1'.repeat(101))).toThrow(new Error('ISBNの文字数が不正です'));
        expect(() => new BookId('1'.repeat(9))).toThrow(new Error('ISBNの文字数が不正です'));
    });
